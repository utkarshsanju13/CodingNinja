...
public class MinimumNumberOfChocalate {
	...
	public static int getMin(int arr[], int N) {
		...
		for (int j = N - 2; j >= 0; j--) {
			...
			dp_old = dp[j];
			dp[j] = Math.max(dp_old, dp_current);
			prev = dp_current;
		}
		// printDp(dp);

		int minChoclateRequired = 0;
		for (int i = 0; i < dp.length; i++) {
			minChoclateRequired = minChoclateRequired + dp[i];
		}

		return minChoclateRequired;
	}
